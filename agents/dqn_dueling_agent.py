...
class DQN_Dueling:
	...
	def init_net(self):
		self.net = DQN_dueling(self.env.observation_space.shape[0], self.hide_size, self.env.action_space.n, self.layer_num)
		self.optimizer=optim.Adam(self.net.parameters(), lr=self.learning_rate)
		self.target_net = DQN_dueling(self.env.observation_space.shape[0], self.hide_size, self.env.action_space.n, self.layer_num)
		self.loss_list = []
	...
	def run(self):
		...
		for epi in range(epi_num):
			...
			if epi % 100 == 0:
				...
				print('DQN Dueling ~~~ epi {}, total reward {}, step num {}'.format(epi, total_reward, step_num))

		print('DQN Dueling ~~ Training Finished !')
