...
class A2C:
	...
	def update_net(self):
		...
		log_prob_actions_v = adv_v*log_prob_v[range(len(states)), actions_v]
		...
		self.epi_loss.extend([loss.data.cpu().numpy()])
	...
	def run(self):
		for epi in range(self.epi_num):
			...
			if epi % 100 == 0:
				...
				print('A2C ~~~ epi {}, total_reward {}, step_num {}'.format(epi, self.total_reward, self.step_num))
